...
class MyLayout(GridLayout):
    def __init__(self, **kwargs):
        ...
        self.video_label = Label(text='', size_hint_x=0.33)
        self.PrevGrid.add_widget(self.video_label)

        self.audio_label = Label(text='', size_hint_x=0.33)
        self.PrevGrid.add_widget(self.audio_label)
    ...
    def detect_input(self, instance):
        global video_input
        global video_label

        cmd = ['v4l2-ctl --list-devices']
        output = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
        video_serial = str(output.communicate()[0])
        #cmd = ['v4l2-ctl --list-devices']
        #output = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
        #video_label = output.communicate()[0]

        #print(video_serial)
        #print(video_label)

        video_in_str_start = video_serial.find('/dev/video0') - 4
        video_in_str_end = video_in_str_start + 11

        # video_in_str_start3 = video_input.find('/dev/video4') - 4
        # video_in_str_end3 = video_in_str_start3 + 11

        video_Label_end = video_serial.find(':') - 2

        output = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
        video_input = str(output.communicate()[0][video_in_str_start:video_in_str_end])[2:-1]

        output = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
        video_label = str(output.communicate()[0][0:video_Label_end])[2:-1]

        self.video_label.text = video_label + ' Input Detected'

        cmd = ['aplay -L']
        output = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
        audio_serial = str(output.communicate()[0])

        audio_in_str_start = audio_serial.find('hw:1') + 11
        audio_in_str_end = audio_in_str_start + 11

        audio_label_start = audio_serial.find('usbstream') + 6
        audio_label_end = audio_serial.find('USB Stream Output') -27

        output = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
        audio_input = str(output.communicate()[0][audio_in_str_start:audio_in_str_end])[2:-1]
        output = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
        audio_label = str(output.communicate()[0][audio_label_start:audio_label_end])[2:-1]


        #print(audio_serial)
        #print(audio_label)
        #print(audio_input)
        #print(audio_in_str_start

        if len(video_label) > 0:
            self.video_label.text = f'Video Detected - {video_label}'
            self.video_label.color = 'white'
        else:
            self.video_label.text = 'No Video Input Detected'
            self.video_label.color = 'red'

        if len(audio_label) > 0:
            self.audio_label.text = f'Audio Detected - {audio_label}'
            self.audio_label.color = 'white'
        else:
            self.audio_label.text = 'No Audio Input Detected'
            self.audio_label.color = 'red'


        detect_video = Clock.schedule_once(self.detect_input, 0.5)
